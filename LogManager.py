# Класс обработки программных логов.
...
class Log:
    ...
    def __init__(self, continuelogs:list=None):
        ...
        saving_file = 'data\\logs\\_last.log'
        
        logging.basicConfig(filename='data\\logs\\_last.log',level=logging.DEBUG,
                            format='[%(asctime)s][%(levelname)s] %(message)s',datefmt='%H:%M:%S:%f',encoding='utf-8')
        
        if continuelogs == None:
            self.logs = []

        else:

            self.logs = continuelogs

            for line in self.logs:
                print(line)

            with open('data\\logs\\_last.log', 'a', encoding='utf-8') as file:
